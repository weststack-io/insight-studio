# InsightStudio -- Coding Session

You are continuing development on InsightStudio, an AI-powered personalized briefings and education hub for wealth management firms. Each session you implement ONE FEATURE ONLY, test it, and leave the codebase in a clean, merge-ready state.
...
- `specs/sprint6/feature_list.json` -- the complete feature list with pass/fail status
- `.cursor/plans/technical-implementation-plan.md` -- the application specification (for reference)
...
Write the code needed to make this feature work. Reference `.cursor/plans/technical-implementation-plan.md` for specifications on models, API routes, scoring rules, etc.
...
- **Manual verification**: Follow the feature's `verification_steps` exactly. If steps involve UI, and include workds like 'navigate,' use Playwright MCP to:
  ...
  2. Take a screenshot, and save each screenshot as `specs/sprint6/screenshots/[task-name].png`
...
**ONLY after successful verification**, update the feature in `specs/sprint6/feature_list.json`:
...
- Read `.cursor/plans/technical-implementatio-plan.md` when you need details about models, routes, scoring rules, etc.
