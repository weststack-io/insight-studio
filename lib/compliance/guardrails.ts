 ...
 */
export function checkRestrictedTerms(content: string): string[] {
 ...
 */
export function checkHighRiskTerms(content: string): string[] {
  const found: string[] = [];
  const lowerContent = content.toLowerCase();
 ...
 */
export function getSuggestedDisclosures(
  content: string,
  highRiskTerms: string[]
): string[] {
  ...
  if (
    highRiskTerms.some((term) =>
      ["cryptocurrency", "crypto", "bitcoin", "ethereum"].includes(term.toLowerCase())
    )
  ) {
  ...
  if (
    highRiskTerms.some((term) =>
      ["derivatives", "options", "futures", "leverage", "margin"].includes(term.toLowerCase())
    )
  ) {
 ...
 */
export function runGuardrails(content: string): GuardrailResult {
  ...
  const requiresReview =
    violations.length > 0 || riskScore > 50 || highRiskTerms.length > 2;
