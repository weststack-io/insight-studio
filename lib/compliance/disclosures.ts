...
import { getPoliciesByType } from "./policy-engine";

export interface DisclosureTemplate {
  id: string;
  name: string;
  content: string;
  contexts: string[]; // When to use this disclosure
  required: boolean; // Whether this disclosure is mandatory
}

// Default disclosure templates
const DEFAULT_DISCLOSURE_TEMPLATES: DisclosureTemplate[] = [
  {
    id: "general",
  ...
  },
  {
    ...
    content:
      "Investments involve risk, including the possible loss of principal. Please consult with a qualified financial advisor before making investment decisions.",
  ...
  },
  {
    ...
    content:
      "Market conditions are subject to change. This analysis reflects conditions at the time of publication and may not be current.",
  ...
  },
  {
    ...
    content:
      "Portfolio performance is based on historical data and may not reflect future results. Individual results may vary.",
  ...
  },
  {
    ...
    content:
      "Cryptocurrency investments are highly speculative and involve significant risk. Prices can be extremely volatile.",
  ...
  },
  {
    ...
    content:
      "Derivatives trading involves substantial risk and is not suitable for all investors. Please consult with a qualified advisor.",
  ...
  },
  {
    ...
    content:
      "This content is not intended as tax advice. Please consult with a qualified tax professional regarding your specific situation.",
  ...
  },
  {
    ...
    content:
      "Estate planning involves complex legal and tax considerations. Please consult with qualified legal and tax professionals.",
 ...
 */
export function getDisclosureTemplates(): DisclosureTemplate[] {
...
export function getDisclosureTemplate(id: string): DisclosureTemplate | null {
  return (
    DEFAULT_DISCLOSURE_TEMPLATES.find((t) => t.id === id) || null
  );
}
...
export async function selectDisclosures(
  ...
  const selected: DisclosureTemplate[] = [];
  const lowerContent = content.toLowerCase();

  // Get tenant-specific required disclosures from policy
  let requiredDisclosureIds: string[] = [];
  if (tenantId) {
    try {
      const disclosurePolicies = await getPoliciesByType(
        tenantId,
  ...
  if (
    cryptoTerms.some((term) => lowerContent.includes(term)) ||
    metadata?.highRiskTerms?.some((term) =>
      cryptoTerms.some((ct) => term.toLowerCase().includes(ct))
    )
  ) {
  ...
  if (
    derivativeTerms.some((term) => lowerContent.includes(term)) ||
    metadata?.highRiskTerms?.some((term) =>
      derivativeTerms.some((dt) => term.toLowerCase().includes(dt))
    )
  ) {
  ...
  if (
    ...
    metadata?.topics?.some((topic) =>
      ["estate", "trust", "inheritance"].some((et) =>
        topic.toLowerCase().includes(et)
      )
    )
  ) {
  ...
  const unique = Array.from(
    new Map(selected.map((d) => [d.id, d])).values()
  );
 ...
 */
export async function getTenantDisclosures(
  tenantId: string
): Promise<DisclosureTemplate[]> {
  // Get tenant-specific required disclosures from policy
  try {
    ...
    if (disclosurePolicies.length > 0) {
      const requiredIds =
        disclosurePolicies[0].config.requiredDisclosures || [];
      return DEFAULT_DISCLOSURE_TEMPLATES.filter((t) =>
