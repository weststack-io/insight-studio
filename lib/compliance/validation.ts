...
export async function validateGeneratedContent(
  ...
  if (citations.length === 0) {
    ...
    const lowConfidenceCitations = citations.filter(
      (c) => c.confidenceScore < 0.5
    );
    if (lowConfidenceCitations.length > 0) {
      warnings.push(
        `${lowConfidenceCitations.length} citation(s) have low confidence scores (< 0.5). Review sources.`
      );
    }
    ...
    if (unlinkedCitations.length > 0) {
      warnings.push(
        `${unlinkedCitations.length} citation(s) are not linked to sources.`
      );
    }
  }
  ...
  if (numbers && numbers.length > 3 && citations.length < 2) {
    warnings.push(
      "Content contains multiple statistics but few citations. Verify all claims are sourced."
    );
  }
  ...
  const foundSuperlatives = superlatives.filter((word) =>
    content.toLowerCase().includes(word)
  );
  if (foundSuperlatives.length > 3) {
    warnings.push(
      "Content contains multiple superlatives. Ensure claims are supported by citations."
    );
  }
...
export async function detectHallucinations(
  ...
  for (const pattern of unsupportedPatterns) {
    ...
    if (matches && matches.length > 0 && citations.length === 0) {
      warnings.push(
        `Content makes claims (${matches.length} instances) but has no citations. Verify sources.`
      );
    }
  }
  ...
  if (specificNumbers && specificNumbers.length > 0 && citations.length < specificNumbers.length) {
    warnings.push(
      `Content contains specific percentages (${specificNumbers.length}) but may not have sufficient citations.`
    );
  }
  ...
  for (const [term1, term2] of contradictoryPairs) {
    if (
      content.toLowerCase().includes(term1) &&
      content.toLowerCase().includes(term2)
    ) {
      suggestions.push(
        `Content may contain contradictory statements (${term1}/${term2}). Review for consistency.`
      );
    }
  }
...
export async function validateContent(
  ...
  const contentValidation = await validateGeneratedContent(
    generatedContent,
    contentId,
    contentType,
    citations
  ...
  const hallucinationCheck = await detectHallucinations(
    generatedContent,
    citations
  );
