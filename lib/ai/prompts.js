...
exports.getLessonPrompt = exports.getExplainerPrompt = exports.getBriefingPrompt = void 0;
function getBriefingPrompt(context) {
    const { type, portfolioData, marketContext, userPreferences, generation, language, sophisticationLevel, } = context;
    const languageNote = language !== "en" ? ` Respond in ${getLanguageName(language)}.` : "";
...
- Asset Classes: ${[
                ...new Set(portfolioData.holdings.map((h) => h.assetClass).filter(Boolean)),
            ].join(", ")}`;
        }
        else {
...
exports.getBriefingPrompt = getBriefingPrompt;
function getExplainerPrompt(context) {
...
exports.getExplainerPrompt = getExplainerPrompt;
function getLessonPrompt(context) {
    const { topic, generation, language, sophisticationLevel, searchContext } = context;
    const languageNote = language !== "en" ? ` Respond in ${getLanguageName(language)}.` : "";
    const sophisticationNote = sophisticationLevel
...
exports.getLessonPrompt = getLessonPrompt;
