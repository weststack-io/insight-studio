...
class SchemaDeployer {
  ...
  private async deployToDatabase(
    ...
    try {
      ...
      // Prisma 7.1 reads datasource from prisma.config.ts
      // Ensure the environment variable is set before Prisma reads the config
      const output = execSync(
      ...
      const migrationsApplied = migrationsMatch
        ? parseInt(migrationsMatch[1], 10)
        : 0;
  ...
  async rollback(
    ...
    try {
      ...
      const statusOutput = execSync(
        "npx prisma migrate status --schema=./prisma/schema.prisma",
        { encoding: "utf-8", env, stdio: "pipe", cwd: process.cwd() }
      );
      ...
      const target =
        targetMigration || this.rollbackInfo.get(databaseName)?.lastMigration;

      if (!target) {
        throw new Error(
          "No target migration specified and no rollback info available. Use: npm run db:rollback <database> <migration-name>"
        );
      }
      ...
      console.log(
        "ℹ️  You need to manually create a new migration that reverses the changes."
      );
      console.log(
        "ℹ️  Alternatively, you can use: npx prisma migrate resolve --rolled-back <migration-name>"
      );
      console.log(`\n💡 To rollback to "${target}", you should:`);
      console.log(
        `   1. Create a new migration that reverses changes after ${target}`
      );
      console.log(
        `   2. Or manually restore from a backup taken before deployment`
      );
    } catch (error: any) {
  ...
  private printSummary(): void {
    ...
    successful.forEach((result) => {
      console.log(
        `   • ${result.database}${
          result.migrationsApplied !== undefined
            ? ` (${result.migrationsApplied} migration(s))`
            : ""
        }`
      );
    });
  ...
  try {
    switch (command) {
      ...
      case "single":
        ...
        if (!dbName) {
          console.error(
            "❌ Error: Database name required for single deployment"
          );
      ...
      case "rollback":
        ...
        if (!rollbackDbName) {
          ...
          console.error(
            "Usage: npm run db:rollback <database-name> [target-migration]"
          );
          process.exit(1);
        }
