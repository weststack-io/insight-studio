...
export default function ExplainersPage() {
  ...
  const handleSearch = async (searchTopic?: string) => {
    ...
    try {
      const response = await fetch(
        `/api/explainers?topic=${encodeURIComponent(topicToSearch)}&language=${language}`
      );
  ...
  const handleGenerate = async (generateTopic?: string, generateLanguage?: Language) => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-primary mb-6">Topic Explainers</h1>
      ...
      {explainer && (
        <ExplainerView
          explainer={explainer}
          onGenerate={(topic, lang) => handleGenerate(topic, lang as Language)}
        />
      )}

      {!loading && !explainer && (
        <div className="text-center py-12 text-gray-500">
          Search for a topic to get started, or click on a popular topic above.
        </div>
      )}
    </div>
  );
