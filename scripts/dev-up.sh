#!/usr/bin/env bash
# InsightStudio -- Start the Next.js dev server in the background.
...
#    Run next dev directly (not via npm run dev) so the PID we record is the
#    actual long-lived server process, not a short-lived npm wrapper that exits
#    early on Node >=24.
: > "$LOG_FILE"
nohup npx next dev >> "$LOG_FILE" 2>&1 &
NEW_PID=$!
